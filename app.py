...
def enable_mfa(username):
    ...
    cursor.execute("UPDATE users SET mfa_secret = ? WHERE username = ?", (mfa_secret, username))
    db.commit()
    db.close()

    return mfa_secret

# Simple user store for staff and members
USERS = {
    "staff": {"password": generate_password_hash("staffpass"), "role": "staff", "mfa_secret": pyotp.random_base32(), "mfa_verified": True},
    "member": {"password": generate_password_hash ("memberpass"), "role": "member", "mfa_secret": pyotp.random_base32(), "mfa_verified": True},
    "pakkarim": {"password": generate_password_hash ("karim"), "role": "staff", "mfa_secret": pyotp.random_base32(), "mfa_verified": True},
    }
...
def generate_qr():
    ...
    session['otp_expiry'] = (datetime.utcnow() + timedelta(seconds=60)).isoformat()  # expires in 60 seconds
...
def verify_otp():
    ...
    if request.method == 'POST':
        ...
        if expiry_time and datetime.utcnow() > datetime.fromisoformat(expiry_time):
            return "❌ OTP expired. Please scan the QR code again."
